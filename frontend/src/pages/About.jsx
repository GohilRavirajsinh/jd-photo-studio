      ...
      <div className="max-w-7xl mx-auto mobile-px">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center mb-20 sm:mb-32">
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h1 className="text-4xl sm:text-5xl md:text-7xl font-bold mb-6 sm:mb-8">Capturing <span className="text-primary">Soul</span> Behind The Lens</h1>
            ...
            <p className="text-zinc-400 text-base sm:text-lg leading-relaxed mb-8 sm:mb-10">
          ...
          <div className="relative mt-12 lg:mt-0">
            ...
            <div className="aspect-square rounded-[2rem] sm:rounded-[3rem] overflow-hidden border border-white/10 glass-card p-3 sm:p-4">
              <img 
                src="https://images.unsplash.com/photo-1493863641943-9b68992a8d07?auto=format&fit=crop&q=80&w=1200" 
                alt="JD Studio Founder" 
                className="w-full h-full object-cover rounded-[1.5rem] sm:rounded-[2.5rem] grayscale hover:grayscale-0 transition-all duration-700"
              />
            </div>
          </div>
        </div>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8 mb-20 sm:mb-32">
          {stats.map((stat, index) => (
            <motion.div 
              ...
              viewport={{ once: true }}
              className="text-center p-6 sm:p-8 glass-card rounded-[1.5rem] sm:rounded-[2rem]"
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 bg-white/5 rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
                {React.cloneElement(stat.icon, { size: 20 })}
              </div>
              <div className="text-2xl sm:text-3xl font-bold text-white mb-1 sm:mb-2">{stat.value}</div>
              <div className="text-zinc-500 text-[10px] sm:text-xs uppercase tracking-widest">{stat.label}</div>
            </motion.div>
          ))}
        </div>

        <div className="bg-primary/5 p-8 sm:p-12 md:p-20 rounded-[2rem] sm:rounded-[3rem] text-center border border-primary/10">
          <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-6 sm:mb-8 italic leading-tight">"We don't take a photograph, we make it."</h2>
          <p className="text-zinc-400 font-display tracking-[0.2em] uppercase text-[10px] sm:text-sm">- Ansel Adams (Our Inspiration)</p>
        </div>
